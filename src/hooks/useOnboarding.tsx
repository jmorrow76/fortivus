...
export const useOnboarding = () => {
  ...
  const getPersonalizedRecommendations = (): PersonalizedRecommendations | null => {
    ...
    const hasGym = onboardingData.available_equipment.includes('full_gym');
    const hasWeights = onboardingData.available_equipment.includes('home_weights');
    const hasBands = onboardingData.available_equipment.includes('resistance_bands');
...
function generateWorkouts(data: OnboardingData, hasGym: boolean, hasWeights: boolean): WorkoutRecommendation[] {
  ...
  const hasMobilityConcern = data.current_challenges.includes('mobility');

  // Workout A - Upper Body / Push Focus
  if (data.fitness_goal === 'build_muscle' || data.focus_areas.includes('strength')) {
    if (hasGym) {
      workouts.push({
  ...
  // Add cardio/conditioning workout for fat loss or energy goals
  if (data.fitness_goal === 'lose_fat' || data.focus_areas.includes('cardio')) {
    workouts.push({
  ...
  // Add mobility/recovery workout if needed
  if (hasMobilityConcern || data.focus_areas.includes('flexibility')) {
    workouts.push({
...
function generateMeals(data: OnboardingData): MealRecommendation[] {
  ...
  let proteinMultiplier = 0.9; // grams per pound target
  let calorieTarget = 2200;
  
  if (data.fitness_goal === 'build_muscle') {
    proteinMultiplier = 1.0;
    calorieTarget = 2600;
  } else if (data.fitness_goal === 'lose_fat') {
    proteinMultiplier = 1.1; // Higher protein during deficit
    calorieTarget = 1900;
  }
...
function generateWeeklySchedule(data: OnboardingData): { day: string; focus: string; intensity: string }[] {
  ...
  const needsRecovery = data.current_challenges.includes('recovery');
  ...
  if (frequency === '1-2') {
    ...
    schedule.push(
      ...
      { day: 'Thursday', focus: 'Active Recovery / Mobility', intensity: 'Low' },
      { day: 'Friday', focus: 'Upper Body', intensity: 'Moderate-High' },
      { day: 'Saturday', focus: 'Lower Body / Conditioning', intensity: 'Moderate' },
      { day: 'Sunday', focus: 'Rest', intensity: 'None' },
    );
  }
